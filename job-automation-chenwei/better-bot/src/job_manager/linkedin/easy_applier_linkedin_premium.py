...
class LinkedInEasyApplier(BaseEasyApplier):
    ...
    async def _fill_up(self, job: Job) -> None:
        ...
        try:
            ...
            for upload_section in upload_sections:
                ...
                if await self._handle_upload_fields(upload_section, job, processed_file_inputs):
    ...
    async def _find_modal_descendants(
        ...
        for variant in selector_variants:
            try:
                ...
                logger.debug(f"Failed finding {description} with {variant.variant} selector: {e}")
                continue
            logger.debug(
        ...
        if text_field:
            ...
            try:
                ...
                for label_selector in label_selectors:
                    selector_type = (
                        "xpath" if label_selector.startswith("xpath=") else "css selector"
                    )
    ...
    async def _click_radio_safely(self, section: Any, radio: Any) -> None:
        ...
        if radio_id:
            ...
            try:
                option_row = radio.locator("xpath=ancestor::div[.//input[@type='radio']][2]").first
